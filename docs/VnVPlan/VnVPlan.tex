...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
  ...
  Jan 5, 2026 & 1.1 & Applied Peer Review \texttt{\#150} \\
  Jan 6, 2026 & 1.2 & Applied Peer Review \texttt{\#147} \\
Jan 11, 2026 & 1.3 & Applied Peer Review \texttt{\#146,\#149} \\
March 9, 2026 & 1.4 & Added Unit Tests \\
April 4, 2026 & 1.5 & TA Feedback \texttt{\#647,\#648} Applied \\
April 5, 2026 & 1.6 & TA Feedback \texttt{\#645} Applied \\
April 6, 2026 & 1.7 & TA Feedback \texttt{\#639} and Peer Review \texttt{\#603} Applied \\
April 21, 2026 & 1.8 & Addresses \texttt{\#651} \\
  \bottomrule
\end{tabularx}
\newpage
\tableofcontents
...
alternative text within that session and copy the final outputs
to their clipboard.
...
history and copying the alternative text.
...
The challenge level of the project is set at a \textit{general} level and
includes two additional components (extras) that support both usability
and technical development.

The first extra is a \textbf{User Testing Report}. This report documents
user tasks, supervisor feedback, internal testing, and a summary of
results and insights. It helps evaluate whether the generated alternative
text is clear, useful, and accessible. The feedback collected from users
guides improvements to both the interface and the quality of the
generated descriptions.

The second extra is a \textbf{Machine Learning Report}, which documents
the dataset, model selection, training iterations, and evaluation of the
AI system used to generate alternative text. The goal of this component
is to improve accessibility by generating accurate descriptions for
physics diagrams. This report also supports further
development by identifying model limitations, tracking improvements
across revisions, and guiding future changes such as dataset expansion
and model optimization.

These extras are important because they address both usability and
technical performance. The Machine Learning Report improves the AI model,
while the User Testing Report ensures the improvements are helpful for
users. Together, they help refine the system and improve the overall
accessibility and effectiveness of the tool.
...
This section outlines the strategies and techniques that will be used for verifying and validating the \textit{Reading4All} system, throughout the various development phases. 
...
However, in the event that roles need to be switched, for example 
if a team member has a busy week and is not as available, responsibilities will be reassigned to another team member. 
To support this transition, team members will have a meeting to discuss any knowledge transfer needed and ensure any questions are answered. This 
will ensure the other team member can confidently take on the new responsibilities/role. 

\begin{table}[H]
    \centering
    \caption{Verification and Validation Responsibility Breakdown}
    \label{tab:data-dictionary-reading4all}
    \begin{tabular}{p{3.0cm} C{3.8cm} p{7.3cm}}
...
\begin{itemize}
  \item \textbf{User Testing}: Stakeholders of this project including
    ...
    uploading multiple images and noting the alternative text
...
\clearpage
\section{Changes from the Original VnV Plan}

This Verification and Validation (VnV) Report follows the structure and
testing strategy outlined in the original VnV Plan. The following updates
reflect changes made to align the report with the final implemented system.

\subsection{Unit Testing Updates}
In the original VnV Plan, unit tests were not applicable due to the early
stage of development. In this report, unit tests are now included for the
implemented modules, reflecting the completed system architecture.

\subsection{Traceability Matrix Updates}
The traceability matrix has been updated to reflect the final mapping
between requirements and test cases. This ensures that all functional and
non-functional requirements are covered and that each test case corresponds
to a requirement.

\subsection{Validation Activity Refinements}
Validation activities such as usability testing, accessibility verification,
and performance evaluation were refined to match the final implementation.
These updates ensure that validation reflects the behavior of the deployed
system.

\subsection{Summary}
Overall, the testing strategy remains consistent with the original VnV Plan,
with updates made to reflect the completed implementation.
...
\begin{enumerate}[label=FR-ST \arabic*., wide=0pt, leftmargin=*]
  ...
  \item{}
    ...
    for each uploaded image that meet the pre-defined evaluation metrics in Appendix ~\ref{appendix:evaluation-metrics}. 
    This includes a sufficiency of description and length of appropriateness score of at least 3, and 
    accessibility/usability and learning impact score of at least 2. \\
    ...
    be evaluated using the evaluation metrics and acceptable ranges defined in 
    Appendix ~\ref{appendix:evaluation-metrics} or expected
...
\begin{enumerate}[label=NFR-ST \arabic*., wide=0pt, leftmargin=*]
  ...
  \item{}
    ...
    reader tests to confirm accessibility compliance. If accessibility issues are identified 
    such as low or insufficient contrast, the UI styling will be revised to meet WCAG 2.1 Level 
    AA requirements and the test will be re-executed.
  ...
  \item \textbf{Interface Style and Branding Verification}. \\[2mm]
    ...
    Norman’s design checklist to ensure compliance. If there are any inconsistencies or usability conflicts,
    the interface design will be adjusted to ensure alignment with Norman’s design principles, and a thorough
    human visual inspection will be done.
  ...
  \item \textbf{Usability Efficiency and Learnability}. \\[2mm]
    ...
    If users take longer than expected or report difficulty completing tasks, 
    feedback will be analyzed to identify usability issues, followed by interface 
    adjustments. A follow-up usability evaluation will be conducted
    to ensure that the issues are fixed.
  ...
  \item \textbf{Alt Text Storage and Personalization Options} \\[2mm]
    ...
    \textbf{Input/Condition:} User chooses to copy
    generated alt text \\
  ...
  \item \textbf{Safety and Timeout Handling} \\[2mm]
    ...
    technical information in messages. If the system fails to handle timeouts safely, 
    error handling logic and mechanisms will be revised and the test will 
    be repeated to confirm safe recovery.
  ...
  \item \textbf{Concurrent Usage and Storage Capacity} \\[2mm]
    ...
    performance and storage usage monitored. If concurrency or storage limits are exceeded, 
    resource management strategies will be adjusted, 
    followed by re-execution of load testing to confirm compliance.
  ...
  \item \textbf{Security, Access, and Network Restrictions} \\[2mm]
    ...
    \textbf{How test will be performed:} One tester and the project supervisor
    (digital accessibility specialist) independently review a representative
    sample of generated outputs. Each output is evaluated for absence of biased
    or offensive language, culturally appropriate references, professional tone,
    and contextual accuracy. Each reviewer records a pass/fail decision for each
    output. The requirement is satisfied if at least 90\% of the reviewed outputs
    are marked as pass by both reviewers. In case of disagreement, the supervisor's
    ...
    \textbf{How test will be performed:} Manual verification will be conducted
    using WCAG 2.1 Level AA and AODA guidelines. The tester evaluates generated
    alt text against WCAG 2.1 Success Criterion 1.1.1 (Non-text Content),
    ensuring descriptions are meaningful, concise, and suitable for assistive
    technologies. Additional checks include clarity of language, avoidance of
    redundant phrases, and proper contextual description. Privacy and file
    handling procedures are reviewed against institutional data policies.
    The requirement is satisfied if all reviewed outputs conform to WCAG 2.1
    Level AA descriptive requirements and no privacy violations are observed.
  ...
  \item \textbf{Environmental and Device Compatibility} \\[2mm]
    ...
    OS platforms. \\
    \textbf{Input/Condition:} System is accessed on various devices and OS platforms. \\
...
\noindent The traceability matrix in Table~\ref{tab:sys-tests} demonstrates complete coverage between the system tests and the requirements in the SRS. Specifically, 100\% of the functional and non-functional requirements are covered by at least one test case, and 100\% of the test cases listed in this document are traceable to one or more functional or non-functional requirements.
...
\textbf{Goal:}
The login component allows users to authenticate using their email and password.
It verifies user credentials with the backend authentication service and redirects
authenticated users to the main upload page.

\textbf{Target requirement(s):} FR6

\begin{itemize}

\item \textbf{Rendering Login Interface}
\begin{itemize}
\item Ensures the login page renders correctly.
\item Verifies that the email and password input fields are visible.
\end{itemize}

\item \textbf{Login Form Validation}
\begin{itemize}
\item Ensures that login attempts with empty fields are rejected.
\item Verifies that an appropriate validation message is displayed.
\end{itemize}

\item \textbf{Credential Input Handling}
\begin{itemize}
\item Confirms that users are able to type and update values in the email and password fields.
\end{itemize}

\item \textbf{Successful Authentication}
\begin{itemize}
\item Ensures valid credentials trigger a backend authentication request.
\item Verifies that authenticated users are redirected to the upload page.
\end{itemize}

\item \textbf{Authentication Error Handling}
\begin{itemize}
\item Ensures backend authentication errors are displayed correctly.
\item Verifies network failures produce an appropriate error message.
\end{itemize}

\end{itemize}


\subsubsection{Signup Component}

\textbf{Goal:}
The signup component allows new users to create an account using their email
address and password. The system validates user inputs and requires a
verification code before account creation is completed.

\textbf{Target requirement(s):} FR6

\begin{itemize}

\item \textbf{Rendering Signup Interface}
\begin{itemize}
\item Ensures the signup form renders correctly.
\item Verifies that email, password, and confirm password fields are present.
\end{itemize}

\item \textbf{Signup Input Validation}
\begin{itemize}
\item Ensures an error message appears if the email field is empty.
\item Ensures passwords shorter than the required length are rejected.
\item Verifies mismatched passwords generate an error message.
\end{itemize}

\item \textbf{Verification Code Request}
\begin{itemize}
\item Ensures the system successfully sends a verification code when valid
signup information is provided.
\end{itemize}

\item \textbf{Verification Code Validation}
\begin{itemize}
\item Ensures the signup process cannot proceed without entering the
verification code.
\end{itemize}

\item \textbf{Successful Signup}
\begin{itemize}
\item Ensures that entering a valid verification code completes account creation.
\item Verifies that a success message is displayed after verification.
\end{itemize}

\end{itemize}


\subsubsection{Authentication Protection Component (RequireAuth)}

\textbf{Goal:}
The authentication protection component ensures that only authenticated
users can access protected pages of the system. If a user is not authenticated,
...
\item \textbf{Authentication Check}
\begin{itemize}
\item Ensures the system checks authentication status before rendering protected content.
\end{itemize}

\item \textbf{Authorized Access}
\begin{itemize}
\item Ensures authenticated users can access protected pages successfully.
\end{itemize}

\item \textbf{Unauthorized Access Handling}
\begin{itemize}
\item Ensures unauthenticated users attempting to access protected routes are redirected to the login page.
\end{itemize}
...
\subsubsection{History Display Component}

\textbf{Goal:}

The history display component presents previously generated images and their
associated alternative text descriptions. This functionality allows users to
review past outputs produced by the system during earlier sessions.

\textbf{Target requirement(s):} FR4, UHR-PIR1

\begin{itemize}

\item \textbf{Rendering History Interface}
\begin{itemize}
\item Ensures that the history page title is displayed when the component loads.
\item Verifies that the navigation button for returning to the main screen is visible.
\end{itemize}

\item \textbf{Displaying History Entries}
\begin{itemize}
\item Ensures that previously generated history entries are displayed correctly.
\item Verifies that each entry shows the associated image file name.
\item Confirms that the generated alternative text is visible to the user.
\end{itemize}

\end{itemize}

\subsubsection{History Preview Interaction}

\textbf{Goal:}

The history preview functionality allows users to select a previously generated
entry and view the corresponding image preview and its alternative text
description.
...
\item \textbf{Image Preview Display}
\begin{itemize}
\item Ensures that selecting a history entry displays the associated image preview.
\item Verifies that the previewed image element appears correctly in the interface.
\item Confirms that the image contains the correct alternative text attribute.
\end{itemize}

\end{itemize}
\subsection{ImageValidation Module}
\textbf{Goal}:
The ImageValidation module ensures that uploaded images meet the systems requirements before they are processed by the
\texttt{Reading4All} system. It verifies that an image file is present in the request, checks whether 
the file type and size meet the defined requirements and confirms that the image is not corrupted and can successfully be opened. 
The following unit tests verify the correctness of this functionality. 

The tests validate proper handling of image uploads, including detection of missing files during uploads, incorrect file types, 
incorrect sized files, and corrupted or inaccessible images. 

\textbf{Target requirements(s)}: FR3, FR4

\begin{itemize}

  \item \textbf{Valid Image Upload}:
  \begin{itemize}
    \item Ensures that the validation module successfully process a correctly formatted image upload request. 
    \item Verifies that the image meets type and size requirements.
    \item Confirms that the module returns \texttt{SUCCESS} when all validation checks pass.
  \end{itemize}

  \item \textbf{Handling Missing Image Upload}:
  \begin{itemize}
    \item Ensures that the validation module correctly identifies when no image is provided in the request. 
    \item Verifies that the image module returns \texttt{MISSING\_IMAGE} when the expected image is missing. 
  \end{itemize}

  \item \textbf{Detecting Invalid File Types}
  \begin{itemize}
    \item  Ensures that the validation module correctly identifies when no an invalid file type is provided in the request. 
    \item Verifies that images whose content\_type is not within the allowed list (PNG or JPEG) that \texttt{INVALID\_FILE\_TYPE} is returned .
  \end{itemize}

  \item \textbf{Detecting Oversized Images}
  \begin{itemize}
    \item Ensures that the module correctly rejects images exceeding the maximum allowed size of 10 MB.
    \item Verifies that the module returns \texttt{FILE\_SIZE\_INVALID} when the inputted file exceeds allowed size.
  \end{itemize}

  \item \textbf{Handling Corrupted or Unauthorized Image Files}
  \begin{itemize}
    \item Ensures that the module correctly detects images that cannot be opened due to corruption or access issues.
    \item Verifies that the module returns  \texttt{UNAUTHORIZED\_ACCESS\_OR\_CORRUPTED} when an image cannot be opened.
  \end{itemize}
\end{itemize}

The test cases for this module can be found \href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_image_validation.py}{here.}

\subsection{BackendController Module}
\textbf{Goal}:
The Backend Controller module is responsible for coordination between the API endpoints and the service modules. 
It receives requests from the application and communicates them to the appropriate service modules, such as the ImageValidation, 
AltText Generation, SessionManagement modules. The controller ensures that requests are routed correctly and 
that the results from each service are properly handled and returned to the user. 

The following unit tests verify that the controller correctly routes requests and returns the expected responses. 
The tests validate the interaction between the controller and service modules, ensuring that image validation, 
alt text generation, alt text history retrievals, alt text editing and user sign up requests are handled correctly.
Furthermore, the alt text editing functionality is implemented as a function that is accessed through the \texttt{BackendController}; 
therefore, it is tested within this section.

Target requirements(s): FR1, FR2, FR3, FR5, FR6

\begin{itemize}
  \item \textbf{Validating Uploaded Images}
  \begin{itemize}
    \item Ensures that the controller correctly calls the image validation module when an image upload request is received.
    \item Verifies that the validation result returned by the \texttt{ImageValidation} service is correctly returned to the caller.
  \end{itemize}

  \item \textbf{Generating Alt Text}
  \begin{itemize}
    \item Ensures that the controller correctly calls the alt text generation module when a valid image upload request is received.
    \item Verifies that the generated alt text returned by \texttt{GenAltText} service is correctly returned to the caller.
    \item Tests that generated alt text is properly post-processed, including removal of extra whitespace, duplicate words and correct capitalization. 
    \item Ensures that successful model generations are properly processed and stored stored in session history using the cleaned version of the alt text. 
  \end{itemize}

  \item \textbf{Retrieving Alt Text History}
  \begin{itemize}
    \item Ensures that the controller correctly calls the the alt text history module when a request to retrieve past alt text is received.
    \item Verifies that the alt text history results returned by the \texttt{GetAltTextHistory} service is correctly returned to the caller.
  \end{itemize}

  \item \textbf{Editing Generated Alt Text}
  \begin{itemize}
    \item Ensures that the controller correctly calls the alt text editing module when a request to edit generated alt text is received.
    \item Verifies that the response returned by the \texttt{EditAltText} service is correctly returned to the caller.
    \item Tests that a valid edit request successfully updates the stored alt text and returns a \texttt{Success} response.
    \item Ensures that invalid requests with missing required fields (such as missing \texttt{entry\_id} or \texttt{edited\_alt\_text}) are rejected and \texttt{INVALID\_REQUEST} is returned.
    \item Ensures that exceptions thrown during the database update process are correctly handled and that a \texttt{UNABLE\_TO\_SAVE} response is returned. 
  \end{itemize}

  \item \textbf{Handling User Sign Up Requests}
  \begin{itemize} 
    \item Ensures that the controller correctly calls the user authentication module when a user sign up request is received.
    \item Verifies that the response returned by the \texttt{UserSignUp} service is correctly returned to the caller.
\end{itemize}
\end{itemize}

The test cases for this module can be found \href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_image_validation.py}{here} for backend controller routing, 
\href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_edit_alt_text.py}{here} 
for alt text editing and \href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_gen_alt_text.py}{here} for alt text generation. 

\subsection{History Navigation}

\textbf{Goal:}

The navigation functionality allows users to return from the history page to the
main upload interface, ensuring smooth interaction between system screens.

\textbf{Target requirement(s):} UHR-PIR1

\begin{itemize}

\item \textbf{Return to Main Screen}
\begin{itemize}
\item Ensures the back button redirects the user to the upload page.
\item Verifies that the navigation function is triggered when the button is clicked.
\end{itemize}

\end{itemize}
\subsection{MainScreen Module}

\subsubsection{HomeScreen Component}

\textbf{Goal:} The HomeScreen component provides the entry point of the system and introduces the purpose of the application. It allows users to understand the system functionality and provides navigation options to sign up or log in.

\textbf{Target requirement(s):} LFR-AR1, LFR-SR1, LFR-SR2, LFR-SR3

\begin{itemize}

\item \textbf{Rendering Home Interface}

\begin{itemize}
\item Ensures that the homepage title ``Physics Alternative Text Generation'' is displayed clearly.
\item Verifies that the homepage subtitle explaining the system purpose is visible to the user.
\end{itemize}

\item \textbf{Navigation Options Visibility}

\begin{itemize}
\item Ensures that the sign-up button for first-time users is displayed on the homepage.
\item Verifies that the login button for returning users is visible and accessible.
\end{itemize}
...
\subsubsection{UploadScreen Component}

\textbf{Goal:} The UploadScreen component enables authenticated users to upload physics diagrams and generate alternative text descriptions using the system’s AI model. The interface supports uploading images, generating alternative text, copying results, and handling user errors.

\textbf{Target requirement(s):} FR1, FR3, UHR-PIR1, LFR-AR4, LFR-SR1

\begin{itemize}

\item \textbf{Rendering Upload Interface}

\begin{itemize}
\item Ensures that the upload page title and subtitle are displayed correctly.
\item Verifies that upload instructions are accessible through ARIA labels.
\item Confirms that the upload area contains accessible labels for screen readers.
\end{itemize}

\item \textbf{Image Upload Functionality}

\begin{itemize}
\item Ensures that the ``Choose File'' button is visible to allow users to upload images.
\item Verifies that JPEG or PNG images can be uploaded successfully.
\item Confirms that drag-and-drop functionality allows users to upload image files.
\item Ensures that users can remove an uploaded image before generating alt text.
\end{itemize}

\item \textbf{Alt Text Generation}

\begin{itemize}
\item Ensures that the system generates alternative text after an image is uploaded.
\item Verifies that generated alt text is displayed in a format compatible with screen readers.
\item Confirms that uploading a new image resets previously generated alternative text.
\end{itemize}

\item \textbf{User Interaction Features}

\begin{itemize}
\item Verifies that users can copy generated alternative text to the clipboard.
\item Confirms that keyboard accessibility allows the file selection dialog to open using the Enter key.
\end{itemize}

\item \textbf{Error Handling}

\begin{itemize}
\item Ensures the system displays an appropriate error message when image upload fails.
\item Verifies that the system displays an error message when alternative text generation fails.
\end{itemize}

\end{itemize}
\subsection{AuthenticationService Module}
\textbf{Goal}:
The Authentication Service module is responsible for managing user authentication and authorization. 
It validates user credentials and communicates with the Supabase authentication service to create new user accounts. 
The module ensures that user emails use the McMaster University domain and that passwords meet the defined length requirements. 

The following unit tests verify that the authentication service correctly validates user credentials, interacts with the 
Supabase authentication service and returns the expected responses for user sign up requests. 
The tests validate that invalid inputs for emails and passwords are correctly handled and that users can successfully sign up when they meet the requirements. 

Target requirements(s): FR 6

\begin{itemize}
  \item \textbf{Creating a New User Account}
  \begin{itemize}
    \item Ensures that the authentication service correctly validates the email and password provided in a user sign up request.
    \item Ensures that the authentication service correctly calls the Supabase authentication service to create a new user account when valid credentials are provided.
    \item Verifies that the response returned by Supabase containing the \texttt{userID and accessToken} are correctly returned to the caller.
  \end{itemize}

  \item \textbf{Validating Email Domain}
  \begin{itemize}
    \item Ensures that the authentication service correctly validates that the email provided in a sign up request uses the McMaster University domain.
    \item Verifies that a \texttt{ValueError} is raised when an email from a different domain is provided.  
  \end{itemize}

  \item \textbf{Validating Password Length}
  \begin{itemize}
    \item Ensures that the authentication service correctly validates that the password provided in a sign up request meets the minimum length requirements.
    \item Verifies that a \texttt{ValueError} is raised when a password is inputted that does not meet the length requirements.
  \end{itemize}

  \item \textbf{Handling Capitalized Email Address Inputs}
  \begin{itemize}
    \item Ensures that the authentication service correctly converts capitalized emails to lowercase before communicating with the Supabase authentication service.
    \item Verifies that the service returns the expected response when a capitalized email is provided.
  \end{itemize}

  \item \textbf{Handling Existing User Sign Up Requests}
  \begin{itemize} 
    \item Ensures that the authentication service correctly handles requests from users who already have an account.
    \item Verifies that the request is not sent to Supabase and that an appropriate error message is returned to the caller. 
\end{itemize}
\end{itemize}
The test cases for this module can be found \href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_auth_service.py}{here.}



\subsection{SessionManagement Module}
\textbf{Goal}:
The Session Management module is responsible for managing session alt text history. 
It retrieves previously generated alt text within a user session and ensures any edits to the generated alt text are properly saved and associated with the correct session.
The module ensures that only the most recent predefined number of alt text generations (with any edits) are retrieved and displayed to the user.  

The following unit tests verify that the module correctly retrieves alt text history for a user's session 
and stores any edits made to the generated alt text. The tests validate that the changes are correctly saved and returned when needed, and that history entries are correctly returned. 
Target requirements(s): FR 3, FR 5

\begin{itemize}
  \item \textbf{Retrieving Alt Text History with a One Entry}
  \begin{itemize}
    \item Ensures that the module correctly retrieves a single alt text history entry for a session ID.
    \item Verifies that the returned entry contains the expected alt text and image.
  \end{itemize}

  \item \textbf{Retrieving Alt Text History with No Entries}
  \begin{itemize}
    \item Ensures that the module correctly handles requests for alt text history when no entries are available.
    \item Verifies that the module returns an appropriate response when no history entries are found for a session ID.
  \end{itemize}

  \item \textbf{Returning History Entries with Edits}
  \begin{itemize}
    \item Ensures that when an edited alt text value exists, the edited alt text is returned in the history rather than the generated version.
    \item Verifies that the returned history entry contains the expected edited alt text and image.
  \end{itemize}

  \item \textbf{Handling empty/NULL edited Alt Text Values}
  \begin{itemize}
    \item Ensures that when an edited alt text value is empty or NULL, the originally generated alt text is returned in the history.
    \item Verifies that the returned history entry contains the expected generated alt text and image.
  \end{itemize}

  \item \textbf{Retrieving Multiple Alt Text History Entries}
  \begin{itemize} 
    \item Ensures that the module correctly retrieves multiple alt text history entries for a session ID.
    \item Verifies that each entry returned contains the expected alt text (edited or generated appropriately) and images.
  \end{itemize}
\end{itemize}
The test cases for this module can be found \href{https://github.com/4G06-CAPSTONE-2025/Reading4All/blob/main/backend/tests/test_alt_text_history.py}{here.}
...
\noindent Table~\ref{tab:survey-targets} summarizes the expected score targets for the quantitative survey questions. These targets define the threshold values the \textit{Reading4All} team expects the system to meet based on the evaluation metrics defined in Appendix~\ref{appendix:evaluation-metrics} and the symbolic constants in the Symbolic Parameters table.

\begin{table}[H]
  \centering
  \caption{Expected Score Targets for Usability Survey Questions}
  \label{tab:survey-targets}
  \begin{tabular}{p{2cm} p{5cm} p{3cm} p{4cm}}
    \toprule
    \textbf{Question} & \textbf{Survey Question Summary} & \textbf{Scale} & \textbf{Expected Target} \\
    \midrule
    UA-Q 4 & Length appropriateness of alt text & Categorical (1--3) & $\geq$ 3 (Proper Length); aligns with \texttt{R\_LENGTH} threshold of 90\% of responses \\
    \addlinespace
    UA-Q 5 & Irrelevant information in alt text & Qualitative (Yes/No + open-ended) & $\leq$ 10\% of users report irrelevant information; aligns with \texttt{R\_SUFFICIENCY} (85\%) \\
    \addlinespace
    UA-Q 6 & Over-detail in alt text & Qualitative (Yes/No + open-ended) & $\leq$ 10\% of users report excessive detail; aligns with \texttt{R\_LENGTH} \\
    \addlinespace
    UA-Q 7 & Accessibility of alt text presentation & Numerical (0--4) & Median $\geq$ 3; minimum $\geq$ 2; aligns with \texttt{R\_USABILITY\_MEDIAN} and \texttt{R\_USABILITY\_MIN} \\
    \addlinespace
    UA-Q 8 & Ease of use and understanding & Numerical (0--4) & Median $\geq$ 3; minimum $\geq$ 2; aligns with \texttt{R\_USABILITY\_MEDIAN} and \texttt{R\_USABILITY\_MIN} \\
    \bottomrule
  \end{tabular}
\end{table}
\begin{enumerate}[label=UA-Q \arabic*., wide=0pt, leftmargin=*]
  ...
  \item \emph{Was the length of the generated alternative text
    ...
    uploaded image.\\
    {\bf Expected Score:} At least \texttt{R\_LENGTH} (90\%) of responses should indicate the length is ``Sufficient'' (score of 3). Responses indicating ``Too Short'' or ``Too Long'' should each account for no more than 10\% of total responses.
  ...
  \item \emph{Did the generated alternative text contain too much
    ...
    {\bf Expected Score:} No more than 10\% of users should report the presence of irrelevant information. This threshold aligns with the \texttt{R\_SUFFICIENCY} target of 85\%.
  ...
  \item \emph{Was the generated alternative text contain over-detailed?
    ...
    effective learning. \\
    {\bf Expected Score:} No more than 10\% of users should report excessive detail. This aligns with the \texttt{R\_LENGTH} target.
  ...
  \item \emph{On a scale of 0-4, is the generated alternative text
    ...
    {\bf Expected Score:} The median response should be $\geq$ \texttt{R\_USABILITY\_MEDIAN} (3), with no individual response falling below \texttt{R\_USABILITY\_MIN} (2). This aligns with the Accessibility/Usability metric in Appendix~\ref{appendix:evaluation-metrics}.
  ...
  \item \emph{On a scale of 0–4, how easy was it for you to use and
    ...
    {\bf Expected Score:} The median response should be $\geq$ \texttt{R\_USABILITY\_MEDIAN} (3), with no individual response falling below \texttt{R\_USABILITY\_MIN} (2). This aligns with the Learning Impact metric defined in Appendix~\ref{appendix:evaluation-metrics}.
