...
\begin{tabularx}{\textwidth}{p{4cm}p{2cm}X}
  ...
  January 17 2026 & 1.1 & Address Peer Review \texttt{\#235}\\
  January 21 2026 & 2.0 & Rev-0 Design Document\\
  April 3 2026 & 2.1 & TA Feedback Fixes on Presentation \texttt{\#650}\\
  April 3 2026 & 2.2 & TA Feedback  \texttt{\#653}\\
  April 4, 2026 & 2.3 & Address Peer Review \texttt{\#236} \\ 
  April 21, 2026 & 2.2 & Address Peer Review \texttt{\#652}\\
  \bottomrule
\end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{YYYY}
    ...
    \textbf{Frontend Modules} & \textbf{System Information Modules}\\
    \midrule
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{0.30\textwidth}Y}
    ...
    \midrule
    \multirow{13}{0.30\textwidth}{Behaviour-Hiding Module [Section (\ref{bhmmain})]} & {DataPreprocess Module (\refstepcounter{mnum}\mthemnum \label{mod:preprocessing_table}) (see \ref{mod:preprocessing})}\\
    ...
    \midrule
    \multirow{6}{0.30\textwidth}{Software Decision Module [Section (\ref{mod:sdmmain})]} & {AIModelTraining Module (\refstepcounter{mnum}\mthemnum \label{mod:modeltraining_table}) (see \ref{mod:modeltraining})}\\
    & {CaptionGeneration Module (\refstepcounter{mnum}\mthemnum \label{mod:captiongeneration_table}) (see \ref{mod:captiongeneration})}\\
    & {InternalMetricCompliance Module (\refstepcounter{mnum}\mthemnum \label{mod:internalmetriccompliance_table}) (see \ref{mod:internalmetriccompliance})}\\
...
The decomposition of the modules was separated by four main subsystems
...
Together, these two backend modules ensure that all authentication, authorization, and session tracking processes are handled transparently and securely, thereby supporting reliability, data protection, and user trust across the Reading4All system. The \texttt{AuthenticationService} 
module handles identity verification and token generation, while the \texttt{SessionManagement} module is solely responsible for managing session state and lifecycle. \texttt{AuthenticationService} does not directly create or delete sessions 
but instead signals \texttt{SessionManagement} to perform these actions.
...
\begin{description}
  ...
  \item[Services:] Produces and formats the final version of the
    accessibility-compliant text for output.
  \item[Implemented By:] Reading4All
...
\begin{description}
  ...
  \item[Services:] Verifies user credentials and generates authentication tokens upon successful login.
    Delegates session creation and termination to the SessionManagement module.
    Validates user identity and access permissions for incoming requests.
...
\begin{description}
  ...
  \item[Services:] Solely responsible for the lifecycle of user sessions, 
  including creation, validation, refresh, and termination of session records. 
  Receives authenticated user information from the AuthenticationService module 
  to initialize or terminate sessions. Maintains session state and history 
  independent of authentication logic.
  \item[Implemented By:] Reading4All
...
This section explains how the module design traces back to the requirements in
our SRS and to the anticipated changes identified earlier in the document.
These traceability tables are included to demonstrate both completeness and the
maintainability of the design.

Tables~\ref{TblFRT} to~\ref{TbltCompR} map the functional and nonfunctional
requirements to the modules responsible for satisfying them. Presenting these
tables in order makes it easier to verify that the requirements have been
covered across the frontend, backend, AI, and support components of the
system.

Table~\ref{TblACT} provides a second form of traceability by mapping the
anticipated changes to the modules that encapsulate them. This supports the
information-hiding decomposition by showing where likely future changes are
isolated in the design.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{HierachyDiagramFinal.pdf}
...
\begin{table}[H]
  ...
  \centering
  \small
  \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{2.8cm}YYY}
    \toprule
    \textbf{Date} &
    \textbf{Task} &
    \textbf{Module Associated} &
    \textbf{Team Member(s) Responsible} \\
    \midrule
    12/25/2025 -- 01/05/2026 & Lock scope of data, set up
    environments, and acquire datasets & None &  All team members \\ 
    01/06/2026 -- 01/11/2026 & Data preprocessing, begin training on
    dataset, metadata labelling & None &  Nawaal, Fiza, Francine \\ 
    ...
    internal metric compliance & InternalMetricCompliance,
    WCAGCompliance &  Nawaal, Fiza, Francine \\ 
    02/02/2026 -- 02/06/2026 & \textbf{Rev0} & None &  All team members \\ 
    02/09/2026 -- 02/13/2026 & User testing for generated alt text &
    None &  All team members \\ 
    ...
    FeedbackLoop, Feedback &  Nawaal, Fiza, Francine \\ 
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{2.8cm}YYY}
    ...
    ImageValidation &  Dhruv, Moly, Francine \\ 
    01/21/2026 -- 01/30/2026 & API testing and integration with
    Frontend and AI systems & Logging  & Dhruv, Moly, Francine \\ 
    02/02/2026 -- 02/06/2026 & \textbf{Rev0} & None &  All team
    members \\ 
    02/09/2026 -- 02/13/2026 & User testing & None &  All team members \\ 
    02/14/2026 -- 02/27/2026 & Revise backend logic if needed after
    user tesing & None &  Dhruv, Moly, Francine \\ 
