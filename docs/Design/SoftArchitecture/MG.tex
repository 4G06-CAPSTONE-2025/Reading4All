...
\begin{enumerate}[label=\textbf{M\arabic*}]
  \item Data Preprocess Module
  \item WCAG Compliance Module
  \item Model Output Module
  \item AI Model Training Module
  \item Caption Generation Module
  \item Feedback Metrics Module
  \item Feedback Integration Module
  \item Classifier/Inference Module
  \item Authentication Service Module
  \item Session Management Module
  \item Image Handling Validation Module
  \item Backend Controller Module
  \item Logging Module
  \item User Interface Interactions Module
  \item Main Screen Module
  \item Show History Screen Module
  \item Log In Screen Module
  \item User Interface Accessibility Module
\end{enumerate}
...
& {User Interface Interactions Module (see \ref{mod: uii})} \\
& {Main Screen Module (see \ref{mod: msm})} \\
& {Show History Screen Module (see \ref{mod: showhm})} \\
& {Log In Screen Module (see \ref{mod: lis})} \\
& {User Interface Accessibility Module (see \ref{mod: uia})} \\
& ? \\
& ? \\ 
...
\subsubsection{User Interface Interfactions Module}
\label{mod: uii}
\begin{description}
  ...
  \item[Services:] Captures user actions from the browser and connects UI controls to application 
...
\subsubsection{Main Screen Module}
\label{mod: msm}
\begin{description}
  ...
  \texttt{renderMain()} gives the main view into the root \texttt{DOM} container and calls the User Interface Accessibility module 
  to assign the main landmark, set initial focus, and announce the screen to screen readers. Throws a \texttt{RenderError}
  if the screen cannot be mounted into the \texttt{DOM}.
  \item[Implemented By:] Reading4All Team 
  \item[Type of Module:] Library
\end{description}

\subsubsection{Show History Screen Module}
\label{mod: showhm}
\begin{description}
  ...
  \item[Services:] Renders the user’s current session history to the screen.
  \texttt{showHistory()} replaces the \texttt{DOM} content with the history view and relies on the User Interface Accessibility module
   to set the main landmark, focus the first history item (if any), and announce the screen.
  Throws a \texttt{RenderError} if content cannot be rendered.
  \item[Implemented By:] Reading4All Team 
  \item[Type of Module:] Library
\end{description}

\subsubsection{Log In Screen Module}
\label{mod: lis}
\begin{description}
  \item[Secrets:] The internal layout composition of the log in screen including username/email and password fields, and 
  a log in button.
  \item[Services:] Renders the system’s Log In screen. \texttt{renderLogIn()} updates the root \texttt{DOM} container to show the log in 
  form, then calls on the User Interface Accessibility module to set focus on the fields on the log in form and announce the screen. 
  Throws a \texttt{RenderError} if content cannot be rendered. 
  \item[Implemented By:] Reading4All Team 
  \item[Type of Module:] Library
\end{description}

\subsubsection{User Interface Accessibility Module}
\label{mod: uia}
\begin{description}
  \item[Secrets:] The internal mechanism for providing screen reader and keyboard navigation compatibility in the browser.
  This includes how ARIA live regions are implemented, how focus is manipulated, and how keyboard navigation is enabled 
  internally.
  ...
  announcements to screen readers using ARIA live regions; and enabling keyboard navigation patterns. The module returns 
  boolean success flags to its callers, and throws an \texttt{AccessibilityError} if accessibility elements 
