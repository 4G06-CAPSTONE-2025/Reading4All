...
\begin{center}
  \begin{tabular}{p{3cm} p{3.5cm} p{4cm} p{4cm}}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    updateModel & Performance-Scores:List & UpdateAck:\Bool & UpdateException \\
    \hline
  \end{tabular}
\end{center}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
validateUser & userToken:String & \Bool & InvalidTokenException \\
\hline
validateImage & image, userToken: & \Bool & InvalidImageException\\
\hline
sendToModel & image, userToken: String & - & AltTextGenerationError\\
\hline 
...
\end{itemize}
  ...
  \hline
  signInUser & credentials & userToken & AuthenticationFailed-Exception \\
  \hline
  signOutUser & userToken:String & Success:\Bool & SessionNotFoundException \\
  \hline
  getCurrentUser & userToken:String & userID & UserNotAuthenticated-Exception \\
  \hline
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
validateImageFile & image, userToken & \texttt{boolean} & InvalidImageType-Exception \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Assumptions}
  ...
  \begin{itemize}
  \item The \texttt{BackendController} module provides the Image Validation module with an image file that is reachable for validation. 
  \item The user has successfully been validated through the user authentication process. 
\end{itemize}
\subsubsection{Access Routine Semantics}

\noindent \textbf{validateImageFile(image, userToken)}
\begin{itemize}
...
\subsubsection{Local Functions}
\begin{itemize}
  \item \textbf{verifyFileType(image)}: Checks that the images file type matches a type found in \texttt{ACCEPTED\_IMAGE\_TYPES}.
  \item \textbf{verifyFileSize(image)}: Checks that the images size falls between \texttt{MIN\_IMAGE\_SIZE} and \texttt{MAX\_IMAGE\_SIZE}.
  \item \textbf{verifyFileReachable(image)}: Checks that the image can be accessed and opened successfully. 
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
logEvent & eventMessage, logType & - & LogWriteException\\
\hline
\end{tabular}
\end{center}
...
\noindent \textbf{logEvent(eventMessage, logType)}:
\begin{itemize}
