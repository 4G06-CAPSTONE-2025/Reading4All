...
\subsection{Uses}
Takes generated captions from the \texttt{CaptionGeneration} module (see Section~\ref{capgen}) and validates them against WCAG 2.1 AA criteria (\citet{WCAG}).
...
\subsection{Uses}
Receives validated captions from the \texttt{WCAGCompliance} module (see Section~\ref{wcagc}) or directly from the 
\texttt{CaptionGeneration} module (see Section~\ref{capgen}) when compliance checks are bypassed.  
...
\subsubsection{Local Functions}
\textbf{splitData(NewFilePath:String) \texttt{->} TestPath:String, TrainPath:String, TrainPath:String}: Divides input data into training and validation portions.
...
\begin{center}
\begin{tabular}{p{3.5cm} p{5 cm} p{3cm} p{3cm}}
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
validateUser & userToken:String & userStatus:\(\mathbb{B}\)  & InvalidTokenException \\
\hline
validateImage & image, userToken: String & imageStatus:\(\mathbb{B}\)  & InvalidImageException\\
\hline
sendToModel & image, userToken: String & None & AltTextGenerationError\\
\hline 
getAltText & image, userToken: String & altText:String & AltTextNotFoundException\\
\hline 
returnToFrontend & altText or errorMsg: String, userID: String & None& UIUnreachableException \\
\hline
updateAltText & altText:String, userID:String & None & SessionStateSaveException\\
\end{tabular}
\end{center}
...
\noindent \texttt{updateAltText (altText, userID)}:
\begin{itemize}
