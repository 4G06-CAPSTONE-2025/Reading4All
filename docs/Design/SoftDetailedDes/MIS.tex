...
\begin{tabularx}{\textwidth}{p{4cm}p{2cm}X}
  ...
  April 21, 2026 & 2.1 & Addresses various issues \texttt{\#657, 661, 655, 659}\\
  \bottomrule
\end{tabularx}
...
\texttt{COLOR\_MODE}: String -- image color mode used before encoding (e.g. "RGB") 
...
\begin{center}
  ...
  \hline
  preprocess & dataset:Map[String, List[String]] & enc: EncodedBatch & InvalidInputException \\
  \hline
  \end{tabular}
\end{center}
...
\texttt{lastCheckedCaption}: String -- Caches the most recently validated caption; initialized to \texttt{""}.\\
\begin{itemize}
  \item transition: lastCheckedCaption := ""; lastReport := []; initialize WCAG rule set and validation thresholds.
\end{itemize}
\texttt{lastReport}: List -- Caches the most recently produced compliance report; initialized to \texttt{[]}.

\subsubsection{Environment Variables}
\texttt{inputImageFiles}: Filesystem paths used to read image files (provided via dataset).\\
\texttt{encodedOutputFile}: Output location for the encoded batch produced by preprocessing.\\
\texttt{PIL/Image Library}: used to open images and convert them to RGB\\
\texttt{processor}:  external processor used to encode images and text.\\
\texttt{tokenizer}:  tokenizer used by \texttt{processor} to identify pad token IDs (i.e. \texttt{processor.tokenizer}).

\subsubsection{Assumptions}
\begin{itemize}
  \item \texttt{dataset} contains a list of image paths and a list of the alt text labels
  \item each image path refers to a readable image file
  \item \texttt{processor} and \texttt{processor.tokenizer} are initialized and available
  \item image paths and alt text lists are aligned by index
\end{itemize}

\subsubsection{Access Routine Semantics}
\noindent \textbf{preprocess(dataset):}
\begin{itemize}
  \item transition:
  \begin{itemize}
  \item Extract image and text data from \texttt{inputImageFiles} and dataset. (images, texts) := extractFields(dataset). 
  \item Load and convert images referenced by \texttt{inputImageFiles} to \texttt{COLOR\_MODE}. images := loadAndConvert(images). 
  \item Normalize associated text inputs. texts := normalizeText(texts). 
  \item Apply encoding via \texttt{processor} to produce \texttt{enc}. enc := encodeImageTextPairs(images, texts). 
  \item Apply label masking to \texttt{enc}. enc := maskLabels(enc). 
  \item encodedOutputFile := enc
  \end{itemize}
  \item output: enc
  \item \textbf{exception:} Raises an \texttt{InvalidInputException} exception if the input examples are missing required fields, contain unreadable image data, or cannot be processed successfully. exc := InvalidInputException if dataset is missing required fields, any image in inputImageFiles is unreadable, or encoding fails.
\end{itemize}

\subsubsection{Local Functions}
\begin{itemize}
  \item extractFields(dataset: Map[String, List[String]]) to (List[String], List[String]): Extracts the image path list and alt text label list from the dataset map.
  \item loadAndConvert(paths: List[String]) to List[Image]: Opens each image file referenced in inputImageFiles via \texttt{PIL\_Image} and converts to \texttt{COLOR\_MODE}.
  \item normalizeText(texts: List[String]) to List[String]: Cleans and standardizes text inputs (e.g., strips whitespace, normalizes Unicode).
  \item encodeImageTextPairs(images: List[Image], texts: List[String]) to EncodedBatch: Applies processor to produce encoded representations of image–text pairs.
  \item maskLabels(enc: EncodedBatch) to EncodedBatch: Sets padding token IDs to -100 in the label tensor to exclude them from loss computation.
\end{itemize}
...
\subsection{Uses}Called by the \texttt{BackendController} module after \texttt{WCAGCompliance} has validated the caption. If \texttt{FinalCaption} is not compliant (i.e. \texttt{checkCompliance} returns a report with violations), \texttt{generateOutput} is not called and \texttt{BackendController} returns an error to the frontend instead. This module does not compute compliance itself.
...
\begin{itemize}
  ...
  \item exception: OutputException if transmission to \texttt{DisplayChannel} fails.
\end{itemize}
...
\subsubsection{Environment Variables}
\subsubsection{Environment Variables}
\texttt{gpu}: Device — GPU execution device (may be unavailable; falls back to \texttt{cpu} if so).\\
\texttt{checkpointDir}: String. Writable directory path for saving checkpoints.\\
\texttt{modelSavePath}: String. Writable file path for saving the final trained model.
...
\begin{itemize}
  \item transition: Trains the model on provided data and on \texttt{Device: = GPU} if it's available, otherwise it uses the \texttt{Device: = CPU}. It also updates the \texttt{trainedModel}.
...
\begin{itemize}
  ...
  \item output: trainedModel written to \texttt{modelSavePath}.
...
\texttt{loadedModel}: 
\begin{itemize}
\item transition: Loads the trained model from \texttt{modelPath} into memory as \texttt{loadedModel}. device := (gpu available $\Rightarrow$ gpu | True $\Rightarrow$ cpu).
\end{itemize}

\subsubsection{Environment Variables}
\texttt{gpu}: Device. GPU execution device (may be unavailable; falls back to \texttt{cpu} if so).\\
\texttt{device}: Device. Selected execution device, set as $(gpu \text{ available} \Rightarrow gpu \mid \mathtt{True} \Rightarrow cpu)$.\\
\texttt{modelPath}: String. File path from which the trained model is loaded.
...
%----------------------------------------------------------
\section{MIS of FeedbackLoop Module} \label{modint}
...
None.
%---------------------------------------------------------
\section{MIS of Feedback Module} \label{modfeedback}
...
\begin{itemize}
  \item Receives captions and performance scores from the \texttt{FeedbackMetrics} module.
  \item Provides stored feedback data to the \texttt{FeedbackLoop} module for model updates.
\end{itemize}

\subsection{Syntax}

\subsubsection{Exported Constants}
\texttt{MAX\_STORAGE}: $\mathbb{N}$ -- Maximum number of feedback entries to store.
...
\subsubsection{State Variables}
\texttt{feedbackDB}: Map[String, FeedbackRecord] -- Stores feedback records keyed by FeedbackID.
...
\subsubsection{Assumptions}
\begin{itemize}
  \item \texttt{initialize} is called before other operations.
  \item Feedback data comes from \texttt{FeedbackMetrics} module.
  \item Storage capacity is sufficient for system operation.
\end{itemize}

\subsubsection{Access Routine Semantics}
\noindent \textbf{initialize():}
\begin{itemize}
  \item transition: Initializes \texttt{feedbackDB} as empty.
  \item output: None.
\end{itemize}

\noindent \textbf{storeFeedback(Caption, Scores):}
\begin{itemize}
  \item transition: Creates new entry in \texttt{feedbackDB} with unique FeedbackID.
  \item output: Returns generated FeedbackID.
  \item exception: Raises \texttt{StorageException} if storage limit exceeded.
\end{itemize}

\noindent \textbf{getFeedback(FeedbackID):}
\begin{itemize}
  \item transition: None.
  \item output: Returns the FeedbackRecord for given ID.
  \item exception: Raises \texttt{RetrievalException} if ID not found.
\end{itemize}

\noindent \textbf{getAllFeedback():}
\begin{itemize}
  \item transition: None.
  \item output: Returns all stored FeedbackRecords.
  \item exception: None.
\end{itemize}
...
\begin{itemize}
  \item transition: Returns the generated alt text or corresponding
    error message to the UserInterfaceInteractions module to be displayed to the user. 
  \item output:  None.
  \item exception: \texttt{FrontendUnreachableException} is raised if
    the Frontend cannot be reached.
\end{itemize}
...
%---------------------------------------------------------
\section{MIS of Logging Module} \label{LogModule}
...
\subsubsection{Assumptions}
  ...
  \begin{itemize}
    ...
    \item User allows system to access files for uploading images.
...
\begin{itemize}
\item transition: The HTTP client sends the image bytes into the backend service which stores the image for that session, and communicates 
...
\begin{itemize} 
\item transition: clipboard is set to \texttt{altText}.
...
\begin{itemize}
\item transition: The HTTP client sends update to backend and stored altText for uploaded image is replaced with editedText.
...
%---------------------------------------------------------
\section{MIS of UserInterfaceInteractions Module}\label{UI_accessibility}
...
\begin{enumerate}
  ...
  For the AI, our team chose 
  to train non Large Language Model (LLM) models. This is because of training constraints 
  (training would be done primarily on our personal computers using open sourced datasets)
  and to address privacy concerns. Additionally, Jing expressed that the learning material 
  would be the intellectual property of the professors/instructors so the optimal choice 
  would be not to save these materials. \\
  The AuthenticationService module was also designed upon getting feedback from Jing. 
  Initially, the team planned to have a single sign on but after discussing, we learned that 
  this may result in extra steps for our potential users. \\
  For design decisions that did not stem from speaking to clients or proxies, our team 
  kept in mind software design principles such as single responsibility. 

  \item \textbf{While creating the design doc, what parts of your other documents (e.g.
  requirements, hazard analysis, etc), it any, needed to be changed, and why?}\\
  When creating the Module Guide and Module Interface Specification documents, specifically for 
  Rev -1, we had to make modifications to our Verification and Validation (VnV) document. Upon 
  creating the SessionManagement module, the team received feedback and discussed that the 
  definition of a "session" was not clear. Therefore, we added a more clear definition for this in 
  our VnV as this is an important terminology in our system. Other than this small addition, 
  nothing had to be changed in the other documents while creating the design document as the team 
  tried to stick with the original requirements written. 


  \item \textbf{What are the limitations of your solution?  Put another way, given
  unlimited resources, what could you do to make the project better? (LO\_ProbSolutions)}
  One limitation of our solution is that the training of the AI model is constrained 
  by available computational resources and data for training the model. With access 
  to more powerful resources such as GPUs, RAM, and greater storage capacity, we could train 
  more complex models in a faster, more efficient manner. Additionally, access to more professors
  data (academic learning materials) would allow the model to learn better and improve accuracy 
  when generating alternative text for images. Furthermore, having human in the loop for captions 
  generated would highly improve how our model understands language. Additionally, a limitation 
  in our current backend system is the database restriction (maximum 500 rows per 
  table) which limits the amount of users that can use the web tool. This also restricts the 
  systems scalability in the future if needed. 

  \item \textbf{Give a brief overview of other design solutions you considered.  What
  are the benefits and tradeoffs of those other designs compared with the chosen
  design?  From all the potential options, why did you select the documented design?
  (LO\_Explores)} \\
  One of the design solutions the team considered was making our own authentication system. 
  However, in our chosen design, the team decided to use a third party authentication service 
  (Supabase) to authenticate our users. This design decision allowed us to easily set up the 
  databases and collaborate during development rather than having everything locally. It is also 
  more secure and helps the team take steps towards preparing for production. If the team had made
  our own authentication system, it would be easier to control during development and have likeliness
  restrictions on the database (i.e. rows, number of users, etc.). \\
  Another design solution considered was using all STEM diagrams for training. In our chosen design,
  however, the team decided to narrow it down to physics diagrams. This is because the open-sourced
  datasets available were to generic and fine-tuning would be a problem unless the scope was narrowed 
  down. Had we used all STEM diagrams, it would be applicable to more students but our team wants to 
  prioritize the accuracy of our generated alt text over diverse consumers. We hope to build 
  on this model given more data in the future. 



  \item \textbf{(After you have implemented another team's module,
      which means this isn't filled in until after the original deadline). 
      ...
      your modules, what was this experience like?  Are there things in your documentation you
      could have changed to make the process go more smoothly for when an
    ``outsider'' completes some of the implementation?} \\

    Implementing another team’s logging module helped us interpret the effectiveness of our peer’s design documentation.
    It also helped us reflect on our own design document to double check any areas of confusion. Overall, the Logger MIS 
    by Team 17 was very well-structured and clearly written, and most of the required functionality was straightforward 
    to implement directly from their specification. We didn’t ask the team questions directly as most of their MIS was 
    mostly clear, however, we made a few assumptions. 

    Some areas required assumptions or interpretation due to a few missing specification or confusion. One major assumption 
    involved log rotation and archival behaviour. The MIS states that logs are “rotated” once they exceed a maximum size 
    but it doesn’t define how to rotate these files when archiving. We assumed a rolling log shere older log files are shifted 
    (e.g., file 1 → file 2 → file 3) within the same log directory. Additionally, although a \texttt{logDir} environment variable is 
    referenced, its path and structure were not specified, so we assumed that all log files reside within a single configurable 
    directory called “/log”. The representation of a \texttt{LogEntry} was another area of interpretation. While the MIS defines its 
    fields abstractly, it does not specify how entries are stored. We assumed a dictionary-based representation serialized to 
    JSON, which is shown in our implementation.

    Some details required assumptions during implementation. For example, we assumed that the limit parameter in \texttt{getRecentLogs} 
    is an integer that specifies how many log entries should be returned. To efficiently retrieve recent logs from large log files, 
    we implemented a helper function (\texttt{\texttt{read\_last\_n\_lines}}) that reads the file in fixed-size chunks instead of loading the entire 
    file into memory. Since the MIS did not specify how file reading should be handled, we assumed a reasonable chunk size 
    and defined a constant \texttt{block\_size} = 4096 bytes as part of the implementation.

    Multiple access to logging was also an assumption and a design consideration that we implemented for the team. We assumed that multiple 
    API requests could call logging simultaneously, which explains our use of locking during archival operations to ensure 
    thread safety. Additionally, the purpose of the \texttt{emitAlert} local function was a little unclear, as severe events already 
    raise errors. Therefore, interpreted \texttt{emitAlert} as an optional feature for notifying external monitoring or alerting systems. 
    However, its role was unclear since errors are already raised through exceptions. Finally, while the MIS is generally thorough, 
    implementation could have been eased by including explicit data types directly in the exported access program table and by 
    further clarifying helper function behavior such as \texttt{formatLogEntry}. 

    We believe theres changes that could be made to the documentation to ensure outsiders can better implement the logger module, with more clarity
    and confidence. Looking at both groups that implemented our modules, it is clear they determined the appropriate format and structure
    of the logs. For example, both groups included a time stamp in their log entries even though this was not explicitly stated in our MIS. Therefore, in order 
    to make the process go more smoothly for outsiders, we should update the documentation to explicitly define the log entry format and required fields. 
    Furthermore, one external group implemented a \texttt{RotatingFileHandler} for log rotation. This highlights that our documentation could have better specified how
    edge cases should be handled, such as the log file exceeding the maximum allowed size. 
    Beyond this, we believe our documentation correctly specified the intended behavior and functionality of the Logging module.
    This is evident in that both external teams were able to correctly implement the modules that could be used the 
    \texttt{Reading4all} system without major assumptions made.
