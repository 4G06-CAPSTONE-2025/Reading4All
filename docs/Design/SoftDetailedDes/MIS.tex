...
\begin{itemize}
  ...
  \begin{itemize}
    \item Extract image and text data from the input examples. 
    \item Load and prepare the input images for downstream processing. 
    \item Clean and standardize the associated text inputs. 
    \item Apply the required preprocessing and encoding operations to the image-text pairs. 
    \item Construct the output data structure in the format required by downstream modules
    \item Apply any label formatting or masking needed for training or inference. 
    \item Write result to encodedOutputFile.
  \end{itemize}
  \item \textbf{output:} Returns the preprocessed and encoded representation of the input examples. out := enc. 
...
\subsection{Uses}
Called by the \texttt{WCAGCompliance} module after a caption has been validated as compliant. If compliance checks are explicitly bypassed, may be called directly by the \texttt{CaptionGeneration} module. This module assumes \texttt{FinalCaption} has already passed compliance checking upstream — it does not perform compliance checks itself.
...
\subsubsection{Environment Variables}
\texttt{cpu}: Device — CPU execution device, always available.\\
\texttt{gpu}: Device — GPU execution device (may be unavailable).\\
