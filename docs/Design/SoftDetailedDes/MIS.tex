...
\begin{center}
  ...
  \begin{tabular}{l l p{7.5cm}}
    ...
    boolean & $\mathbb{B}$ & a logical value representing either \texttt{true} or \texttt{false}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\subsection{Uses}
Processes and standardizes image data through normalization and validation 
so it can be correctly used by both the AIModelTraining and CaptionGeneration modules.
...
\begin{center}
  \begin{tabular}{|l|l|l|l|}
    ...
    \hline
    filterInput & Image & PreprocessedImage & InvalidInputException \\
    \hline
  \end{tabular}
\end{center}
...
\subsection{Uses}
Takes generated captions from the CaptionGeneration module (see Section~\ref{capgen}) and validates them against WCAG 2.1 AA criteria (\citet{WCAG}).
...
\begin{center}
  \begin{tabular}{|l|l|l|l|}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    checkCompliance & CaptionText & ComplianceReport & ValidationException \\
    \hline
  \end{tabular}
\end{center}
...
\subsection{Uses}
Receives validated captions from the WCAGCompliance module (see Section~\ref{wcagc}) or directly from the CaptionGeneration module (see Section~\ref{capgen}) when compliance checks are bypassed.  
...
\begin{center}
  \begin{tabular}{|l|l|l|l|}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    generateOutput & FinalCaption & DisplayConfirmation & OutputException \\
    \hline
  \end{tabular}
\end{center}
...
\subsection{Uses}
Takes preprocessed data from the DataPreprocess module and trains a model. 
Incorporates feedback updates from the FeedbackIntegration module, which consumes performance metrics and adjusts model parameters to improve future training iterations.
...
\hline
saveCheckpoint & progress: Integer & None & WriteErrorException \\
\hline
\end{tabular}
\end{center}
...
\noindent initializeModel(configPath):
\begin{itemize}
...
\noindent trainModel(trainingData):
\begin{itemize}
...
\noindent saveCheckpoint(progress):
\begin{itemize}
...
\subsubsection{Local Functions}
splitData(NewFilePath:String) \texttt{->} TestPath:String, TrainPath:String, TrainPath:String: Divides input data into training and validation portions.
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
loadModel & modelPath: String & None & ModelLoadException \\
\hline
...
\subsection{Uses}
Receives generated captions from the WCAGCompliance module, evaluates their overall quality and accessibility.
...
\subsection{Uses}
Consumes performance scores from the FeedbackMetrics module.
...
\noindent updateModel():
\begin{itemize}
  \item transition: updates \texttt{modelState} and signals AIModelTraining to (re)train with revised parameters.
...
\end{itemize}
  ...
  \hline
  getCurrentUser & userToken:String & userID & UserNotAuthenticatedException \\
  \hline
  refreshToken & userToken:String & newToken & TokenRefreshException \\
  \hline
  \end{tabular}
  \end{center}
  \subsection{Semantics}
