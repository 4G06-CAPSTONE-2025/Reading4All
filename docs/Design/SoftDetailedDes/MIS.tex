...
so it can be correctly used by both the \texttt{AIModelTraining} and \texttt{CaptionGeneration} modules.
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
validateUser & userToken & \Bool & InvalidTokenException \\
\hline
validateImage & image, userToken & \Bool & InvalidImageException\\
\hline
sendToModel & image, userToken & - & AltTextGenerationError\\
\hline 
getAltText & image, userToken & altText & AltTextNotFoundException\\
\hline 
returnToFrontend & altText or errorMsg, userID & -& FrontendUnreachable-Exception \\
\end{tabular}
\end{center}
