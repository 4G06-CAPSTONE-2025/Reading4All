...
\begin{table}[hp]
  ...
  \begin{tabularx}{\textwidth}{llX}
    ...
    November 24th 2025 & Casey Francine Bulaclac & Implement Peer Review \#155,\#151 \\
    April 3rd 2026 & Moly Mikhail & Implement TA Feedback \#649\\
    \bottomrule
  \end{tabularx}
\end{table}
...
This hazard analysis identifies and evaluates risks to ensure that the system provides consistent accessibility gains while not mistakenly introducing new obstacles.
...
\begin{itemize}
    ...
    \item \textbf{User Insensitivity:} Alt text that is excessively technical, overly simplified, or inconsistent may not meet the needs of students. This may reduce usability and participation among students, teachers, and accessibility professionals.
...
The purpose of this hazard study is to systematically identify these hazards, estimate their potential impact, and develop mitigation strategies. This minimizes losses associated with time, trust, resources, and accessibility outcomes, ensuring that the tool contributes positively to equitable education.
...
and integrates seamlessly with screen readers to improve accessibility for
      ...
      \item \textbf{Key Functions:}
        \begin{itemize}
          \item Enable users to upload images through an accessible
            web interface.
          \item Display generated alt text and allow users to edit, and 
            copy the text.
...
\begin{itemize}
  ...
  \item \textbf{Assumption 5: Network Stability.} The system is assumed to operate in an environment 
  with stable and sufficient network connectivity to support uninterrupted communication 
  between the different system components. Network interruptions are assumed to be 
  minimal and not severe enough to impact system safety.
\end{itemize}
...
\begin{landscape}
    ...
    HA-1 & Frontend UI &
    Not all user interface components can be controlled using a keyboard &
    ...
    Insufficient training data diversity; lack of output validation &
    Introduce a post-processing validation step where generated alt text is checked against accessibility standards (e.g., AODA, WCAG 2.1) and team-defined evaluation metrics before being presented to the user; outputs that fail these checks are blocked or flagged for review &
    SR-PR~2 \\ \hline
    ...
    Auto-delete temp files; periodic cleanup; log storage usage; generate an error log and notify a system administrator when a deletion job fails &
    SR-PR~1 \\ \hline
    ...
    HA-11 & Alt Text Generation Model &
    Model performance degrades over time as new diagram types or formats emerge &
    Reduced accuracy, misleading alt text, or failure to interpret modern content; users lose trust in system &
    Outdated training data; distribution shift; lack of periodic retraining or evaluation &
    Schedule periodic model evaluations; monitor quality metrics; update training dataset; retrain model on newly collected examples; implement feedback loop for reporting inaccurate outputs &
    PR-PAR~1, SR-IM~2 \\
...
\begin{enumerate}[label=PR-SR-HA \arabic*., wide=0pt, leftmargin=*]
  ...
  \item \emph{The system must safely exit when a timeout occurs and ensure that no user data or incomplete alt text is stored or shown to the user.}\\[2mm]
    ...
    {\bf Priority:} High\\
    {\bf Hazard Analysis Connected}: HA4
  \item \emph{The system must ensure that all messages notifying the user of failure do not reveal any system code or data. }\\[2mm]
