...
\hypersetup{
  ...
  linkcolor=red,          % colour of internal links (change box colour
  % with linkbordercolour)
...
T\_UI\_RESP & 300 milliseconds (ms) \\
R\_SUFFICIENCY & 85\% \\
R\_LENGTH & 90\% \\
...
IMG\_SIZE\_BIG & 10 MB \\
IMG\_SIZE\_SMALL & 2 MB \\
TLS\_VERSION & 1.2 \\ 
FILE\_DELETE\_TIME & 60 s \\
FILE\_TYPES & .pdf, .svg, .webp \\
...
April 1, 2026 & 1.1 & Address TA Feedback \texttt{\#638}, \texttt{\#646} \\
April 3, 2026 & 1.2 & Address TA Feedback \texttt{\#644}\\
April 4, 2026 & 1.3 & Added explicit table references, corrected table numbering, and updated formatting to ensure all tables are properly labeled and referenced. \\
April 6, 2026 & 1.4 & Converted large tables to longtable to prevent footer overlap; added introductory text to Sections 4 and 5; replaced bitmap figures where needed. \\
April 22, 2026 & 1.5 & Added requirements traceability matrix (Section~6.4) to address TA Feedback \texttt{\#643}. \\
\bottomrule
\end{tabularx}
...
context, restrictions, and expected behaviour in order to ensure that
...
\subsection{Organisation of Document}
...
\end{itemize}
This organisation ensures logical flow from project background to
...
\begin{enumerate}[label=MD-SL \arabic*., wide=0pt, leftmargin=*]
  ...
  \item \emph{The solution must be implemented as a web tool}\\[2mm]
    {\bf Rationale:} This requirement was specified by our supervisor to ensure 
    the solution supports automated testing 
    against the WCAG standards. This helps us ensure accessibility for users
    and provides users with a convenient interface to generate alternative text.\\
...
\begin{enumerate}[label=MD-SC \arabic*., wide=0pt, leftmargin=*]
  \item \emph{The final product must be completed and tested by the
    ...
    \item \emph{High-priority functional requirements must be implemented before system integration begins.}\\[2mm]
    {\bf Rationale:} This is to ensure that the final product is
    functional and meets all requirements
    at the end of the academic year.\\
    {\bf Fit Criterion:} High-priority requirements (FR1, FR2, FR3, FR6) are implemented by the end of January 2026. \\
    {\bf Priority:} High.

    \item \emph{Medium-priority functional and usability requirements must be implemented after core 
    functionality is established.}\\[2mm]
    {\bf Rationale:} Once core features are complete, additional functionality can be layered to 
    improve usability and system completeness.\\
    {\bf Fit Criterion:} Medium-priority requirements (FR4, FR5 and usability/accessibility enhancements) 
    are implemented by the end of February 2026. \\
    {\bf Priority:} Medium.

    \item \emph{Performance, robustness, and system testing requirements must be completed prior to final system delivery.}\\[2mm]
    {\bf Rationale:} Ensuring system reliability and performance before final delivery reduces the risk of 
    failure and improves user experience.\\
    {\bf Fit Criterion:} Performance and robustness requirements are implemented and full system testing is completed by March 2026. \\
    {\bf Priority:} High.

     \item \emph{Final system validation and documentation must be completed before project submission.}\\[2mm]
    {\bf Rationale:} Proper validation and documentation ensure the system meets all requirements and is 
    ready for deployment and evaluation.\\
    {\bf Fit Criterion:} Final validation and documentation are completed before April 2026. \\
    {\bf Priority:} High.
...
\begin{enumerate}[label=MD-BC \arabic*., wide=0pt, leftmargin=*]
  \item \emph{The project budget must include compensation for user
    ...
    {\bf Rationale:} Implementing core features first ensures that the system has a stable foundation before 
...
This section defines the terminology, acronyms, and naming conventions used throughout the document to ensure consistent interpretation of requirements.
...
This section outlines known facts, assumptions, and business rules that influence the design and scope of the Reading4All system.
...
The business rules regarding the system are as follows:
\begin{itemize}
  \item Only authenticated users (McMaster Students) can access system functionality.
  \item The system must only accept supported image formats (e.g., JPEG, PNG).
  \item Generated alternative text must comply with WCAG 2.1 Level AA standards.
  \item User session data must be temporary and deleted after session termination.
  \item The system must not store or expose personally identifiable information (PII).
  \item Generated outputs must not contain harmful, biased, or offensive content.
\end{itemize}
...
Table~\ref{tab:traceability} maps each functional and non-functional requirement to the Product Use Case(s) (PUC) that implement it, the Business Use Case(s) (BUC) it traces back to, and its priority.

\begin{longtable}{p{2.2cm}p{5.5cm}p{2.2cm}p{2.0cm}p{1.5cm}}
\caption{Requirements Traceability Matrix}
\label{tab:traceability} \\

\toprule
\textbf{Req. ID} & \textbf{Summary} & \textbf{PUC(s)} & \textbf{BUC(s)} & \textbf{Priority} \\
\midrule
\endfirsthead
...
\midrule
FR 1   & Accept JPEG/PNG diagrams up to IMG\_SIZE\_BIG              & PUC 2              & BUC-2         & High   \\
FR 2   & Generate alternative text for uploaded images               & PUC 3              & BUC-4         & High   \\
FR 3   & Output alt text compatible with screen readers              & PUC 3              & BUC-4         & High   \\
FR 4   & Allow users to edit outputted alt text                      & PUC 4              & BUC-4         & High   \\
FR 5   & Store and display images and alt texts within session        & PUC 5              & BUC-5         & Medium \\
FR 6   & Validate users via McMaster SSO on login                    & PUC 1              & BUC-1         & High   \\
\midrule

\multicolumn{5}{l}{\textit{Look and Feel Requirements}} \\
\midrule
LFR-AR 1 & Text resizable to MAX\_ZOOM\_PERCENTAGE without loss of function & PUC 1--5        & BUC-0         & High   \\
LFR-AR 2 & Colour not sole method to convey information               & PUC 1--5           & BUC-0         & High   \\
LFR-AR 3 & Minimum MIN\_CONTRAST\_RATIO contrast ratio for all text    & PUC 1--5           & BUC-0         & High   \\
LFR-AR 4 & Alt text provided for all non-text UI content               & PUC 1--5           & BUC-0         & High   \\
LFR-SR 1 & Simple, modern design with $\leq$3 colours                  & PUC 1--5           & BUC-0         & High   \\
LFR-SR 2 & McMaster branding while meeting WCAG contrast               & PUC 1--5           & BUC-0         & High   \\
LFR-SR 3 & Interface follows Don Norman's design principles            & PUC 1--5           & BUC-0         & High   \\
\midrule

\multicolumn{5}{l}{\textit{Usability and Humanity Requirements}} \\
\midrule
UHR-EUR 1 & Upload and generate alt text in $\leq$MAX\_UPLOAD\_STEPS steps & PUC 2, PUC 3   & BUC-2, BUC-4  & High   \\
UHR-EUR 2 & Returning users complete core tasks within MAX\_MINUTES unaided & PUC 1--3      & BUC-0         & Medium \\
UHR-EUR 3 & Textual feedback within 1\,s of user action                 & PUC 2, PUC 3       & BUC-2, BUC-4  & High   \\
UHR-EUR 4 & $\geq$USERS\_SUCCESS\_PERCENT complete tasks error-free; errors explained within MAX\_ERROR\_RECOVER & PUC 2, PUC 3 & BUC-0 & High \\
UHR-PIR 1 & Copy alt text individually or in bulk from history          & PUC 4, PUC 5       & BUC-5         & High   \\
UHR-LR 1  & $\geq$LEARNING\_PERCENT of first-time low-vision users succeed within MAX\_LEARNING\_MINUTES & PUC 1--3 & BUC-0 & High \\
UHR-UPR 1 & Only essential information shown; no technical details      & PUC 1--5           & BUC-0         & High   \\
UHR-AR 1  & System meets WCAG 2.1 Level AA                              & PUC 1--5           & BUC-0         & High   \\
UHR-AR 2  & All main functions navigable by keyboard                    & PUC 1--5           & BUC-0         & High   \\
\midrule

\multicolumn{5}{l}{\textit{Performance Requirements}} \\
\midrule
PR-SL 1    & Alt text within T\_ALT\_GEN\_SMALL ($\leq$IMG\_SIZE\_SMALL) / T\_ALT\_GEN\_LARGE ($\leq$IMG\_SIZE\_BIG) & PUC 3 & BUC-4 & High \\
PR-SL 2    & Interactive elements respond within T\_UI\_RESP               & PUC 1--5           & BUC-0         & Medium \\
PR-SCR 1   & No PII stored; images deleted after session                 & PUC 2, PUC 3       & BUC-2, BUC-4  & High   \\
PR-SCR 2   & 0\% of outputs contain offensive or biased language         & PUC 3              & BUC-4         & High   \\
PR-SCR 3   & Interface adheres to WCAG 2.1 AA                            & PUC 1--5           & BUC-0         & High   \\
PR-SR-HA 1 & User notified with retry option on generation timeout       & PUC 3              & BUC-4         & Medium \\
PR-SR-HA 2 & System safely exits on timeout; no partial alt text stored  & PUC 3              & BUC-4         & High   \\
PR-SR-HA 3 & Error messages contain no system code or internal data      & PUC 1--5           & BUC-0         & High   \\
PR-PAR 1   & $\geq$R\_SUFFICIENCY of outputs rated ``Sufficient'' or higher & PUC 3             & BUC-4         & High   \\
PR-PAR 2   & $\geq$R\_LENGTH of outputs rated ``Proper Length''           & PUC 3              & BUC-4         & Medium \\
PR-PAR 3   & Median usability $\geq$R\_USABILITY\_MEDIAN; no output below R\_USABILITY\_MIN & PUC 3 & BUC-4 & Medium \\
PR-RFT 1   & Invalid inputs trigger error within T\_ERROR\_HANDLE         & PUC 2              & BUC-2         & High   \\
PR-RFT 2   & Backend recovers within T\_RECOVERY after fault              & PUC 3              & BUC-4         & High   \\
PR-CR 1    & $\geq$CAP\_CONCURRENT concurrent requests at $\leq$10\,s    & PUC 2, PUC 3       & BUC-4         & Medium \\
PR-CR 2    & Temporary storage supports up to CAP\_STORAGE               & PUC 2, PUC 5       & BUC-5         & Low    \\
PR-SER 1   & New ML models integrable without restructuring               & PUC 3              & BUC-4         & Medium \\
PR-LR 1    & Minor updates require $\leq$MAINT\_TIME                     & PUC 1--5           & BUC-0         & Medium \\
PR-LR 2    & Compatibility with previous SFWR\_RELEASES releases          & PUC 1--5           & BUC-0         & Low    \\
\midrule

\multicolumn{5}{l}{\textit{Security Requirements}} \\
\midrule
SR-AR 1  & Access restricted to McMaster SSO authenticated users       & PUC 1              & BUC-1         & High   \\
SR-AR 2  & All actions tied to authenticated session                   & PUC 1--5           & BUC-1         & Medium \\
SR-IR 1  & All communication over HTTPS (TLS TLS\_VERSION+)            & PUC 1--5           & BUC-0         & High   \\
SR-IR 2  & Uploaded images remain unmodified (hash-verified)           & PUC 2, PUC 3       & BUC-2, BUC-3  & High   \\
SR-PR 1  & Images deleted within FILE\_DELETE\_TIME after session ends  & PUC 2, PUC 5       & BUC-2, BUC-5  & High   \\
SR-PR 2  & Alt text filtered for PII and inappropriate content         & PUC 3              & BUC-4         & Medium \\
SR-AU 1  & Logs record auth events, uploads, generation (no content)   & PUC 1--5           & BUC-0         & Medium \\
SR-AU 2  & Audit logs restricted to authorized administrators          & PUC 1--5           & BUC-0         & Medium \\
SR-IM 1  & Only FILE\_TYPES accepted; scripts rejected                  & PUC 2              & BUC-2         & High   \\
SR-IM 2  & Access blocked outside NETWORK\_SOURCE\_POLICY              & PUC 1              & BUC-1         & High   \\
\bottomrule
...
\begin{figure}[H] % or [htbp] + \FloatBarrier below
  \centering
  \includegraphics[width=\textwidth]{images/work-context-diagram.pdf}
...
\subsection{Specifying Business Use Cases (BUC)}

To maintain alignment between the business use cases and the work
partitions defined in Section~6.3, the system functionality is
described using one primary, end-to-end Business Use Case (BUC),
supported by several smaller BUCs. The primary BUC captures the
overall user goal, while the supporting BUCs isolate and describe
each major system interaction identified in the work partitioning.

\subsubsection*{BUC-0: Generate Alternative Text for Technical Diagrams (Primary)}

The project has one primary business use case, which aims to achieve
the goal of providing users with visual and cognitive impairments an
efficient and accessible way to generate accurate alternative text
for academic images and figures.
...
\begin{enumerate}
  ...
  \item The system analyzes the uploaded file(s), performs OCR to
  extract any visible text, and generates alternative text describing
  each image or figure
  \item Screen readers use the generated alternative text to convey
  the uploaded image
  \item The generated alternative text can be edited, and copied by the user
...
\subsubsection*{BUC-1: Authenticate User}

\textbf{Goal:} Allow authorized McMaster users to access the system.\\
\textbf{Related Work Partition:} Login

\subsubsection*{BUC-2: Upload Images or Figures}

\textbf{Goal:} Allow users to upload technical diagrams for alternative
text generation.\\
\textbf{Related Work Partition:} Upload Images/Figures

\subsubsection*{BUC-3: Extract Embedded Text from Images}

\textbf{Goal:} Extract visible text from uploaded images to support
accurate alternative text generation.\\
\textbf{Related Work Partition:} Optical Character Recognition (OCR)
Text Extraction

\subsubsection*{BUC-4: Generate Alternative Text}

\textbf{Goal:} Generate descriptive alternative text using the
uploaded image and extracted OCR data.\\
\textbf{Related Work Partition:} Generate Alternative Text

\subsubsection*{BUC-5: View Session History}

\textbf{Goal:} Allow users to review images and alternative text
generated during the current session.\\
\textbf{Related Work Partition:} View History


\section{Business Data Model and Data Dictionary}
This section describes the structure and organisation of the data flow throughout the system. It explains how the system’s components interact with the stored data, ensuring a consistent and well-defined understanding of the information processed by the tool.
...
\begin{figure}[h!]
    ...
    \includegraphics[width=0.85\textwidth]{images/business-data-model.pdf}
...
Table~\ref{tab:data-dictionary-reading4all} presents the data dictionary for the Reading4All system and describes the key components and their roles.

\begin{longtable}{p{3.6cm}p{7.4cm}p{3.0cm}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/Product_Boundary_Diagram.pdf}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{1.3cm}|p{2.5cm}|p{3.5cm}|p{4cm}|p{2.6cm}}
    ...
    \midrule
    PUC 4 & Copy Alt Text & McMaster Student and Faculty, UI, Backend Component &
    User decision to copy (input), text copied to
    clipboard & UHR-PIR 1\\
    \midrule
...
\begin{quote}
\textbf{Trigger:} User decides to modify the generated alternative text before copying it.\\
\textbf{Preconditions:}
\begin{itemize}
...
\textbf{Output:} The edited alternative text is displayed to the user and stored within the session, to be copied later.
\end{quote}
\textbf{PUC 5: Copy the Generated Alternative Text }
\begin{quote}
\textbf{Trigger:} User selects copy after generating alternative text.\\
\textbf{Preconditions:}
\begin{itemize}
  \item System has successfully generated alternative text for an uploaded image.
...
\textbf{Input:} User decision to copy.\\
\textbf{Output:} Text is copied to the users clipboard.
\end{quote}
...
\begin{enumerate}[label=FR \arabic*., wide=0pt, leftmargin=*]
  \item \emph{The system must accept technical diagrams in the format
   ...
   {\bf Fit Criterion:} The system accepts JPEG and PNG images with file sizes $\leq$ IMG\_SIZE\_BIG. 
   Unsupported file formats or oversized files are rejected 
   with an accessible error message displayed within T\_ERROR\_HANDLE.\\
    {\bf Priority:} High
  \item \emph{The system shall generate alternative text of uploaded
    ...
    alternative-text. \\
    {\bf Fit Criterion:} For a predefined evaluation dataset, 
    at least R\_SUFFICIENCY of generated alternative text outputs are rated as “Sufficient” or higher according to the Evaluation Metrics Summary (Appendix~\ref{sec:evaluation-metrics}).\\
    {\bf Priority:} High
  ...
  \item \emph{The system shall allow users to edit the outputted
    ...
    {\bf Fit Criterion:} Users can modify, save, and view edited alternative text within the same session, with changes reflected immediately in the interface and confirmed through visible feedback.\\
    {\bf Priority:} High
  \item \emph{The system shall store and display all inputted images
    ...
    {\bf Fit Criterion:} All uploaded images and their corresponding generated or edited alternative text remain accessible to the user for the duration of the active session and are removed upon session termination.\\
    {\bf Priority:} Medium
  \item \emph{The system must validate users during login to confirm
    ...
    {\bf Fit Criterion:} Only users authenticated through McMaster University Single Sign-On (SSO) can access system functionality; unauthenticated users are denied access and redirected to the login page.\\
    {\bf Priority:} High
\end{enumerate}
...
\begin{enumerate}[label=LFR-AR \arabic*., wide=0pt, leftmargin=*]
  \item \emph{The system must allow all text on the interface to be resized up to MAX\_ZOOM\_PERCENTAGE, without any loss of functionality or content. }\\[2mm] 
    ...
    {\bf Priority:} High
  \item \emph{The system must not use colour as the only method to
    provide information, indicate actions or prompt user input.}\\[2mm]
    {\bf Rationale:} Users with colour vision deficiencies or other
    visual impairments may not detect colour differences accurately.
    This also ensures the system meets WCAG 2.1 Success Criterion
    1.4.1 Use of Colour.\\
    {\bf Fit Criterion:} Any use of colour communicates information to
  ...
  \item \emph{The system must ensure sufficient contrasts of text and images of text.}\\[2mm] 
    {\bf Rationale:} Sufficient colour contrast is important as it enables users with low vision or colour vision deficiencies to easily read any system text. This also ensures the system meets WCAG Success Criterion 1.4.3 Contrast (Minimum)\\
...
\begin{enumerate}[label=LFR-SR \arabic*., wide=0pt, leftmargin=*]
  \item \emph{The system interface must follow a simple and modern
    ...
    maximum of three colours, consistent font styles and sizes, as
...
\item \emph{The system interface must allow users to choose how generated alternative text is copied.}\\[2mm] 
    {\bf Rationale:} Allowing users to copy alternative text immediately after generation or in bulk from their session history users supports different user workflows.
    {\bf Fit Criterion:} After generating the alternative text users can either copy individual entries at generation time or copy them from their session history, and the system successfully copies the selected text to the clipboard.\\
    {\bf Priority:} High
\end{enumerate}
...
While the project follows the official course deliverable schedule,
the development of Reading4All is organized into implementation phases
that are tailored specifically to the project scope and requirements.
Each phase reflects concrete development, validation, or delivery
activities and clearly identifies what system functionality is
addressed during that period.

\subsubsection*{Phase 1: Requirements Finalization and Planning (Early January)}
\textbf{Planned Activities:}
\begin{itemize}
  \item Finalize functional and non-functional requirements
  \item Resolve feedback from document and peer reviews
  \item Establish implementation priorities and constraints
\end{itemize}

\subsubsection*{Phase 2: System Design and Architecture (Mid January)}
\textbf{Planned Activities:}
\begin{itemize}
  \item Define system architecture and component boundaries
  \item Design frontend, backend, and model integration interfaces
  \item Prepare implementation and testing plans
\end{itemize}

\subsubsection*{Phase 3: Core Implementation and Initial Validation (February)}
\textbf{Planned Activities:}
\begin{itemize}
  \item Implement core functionality (image upload, OCR, alt text generation)
  \item Integrate accessibility features into the user interface
  \item Conduct informal validation and early testing
\end{itemize}

\subsubsection*{Phase 4: Verification, Validation, and Refinement (March)}
\textbf{Planned Activities:}
\begin{itemize}
  \item Perform formal verification and validation activities
  \item Conduct usability and accessibility testing
  \item Refine system outputs based on evaluation metrics and feedback
\end{itemize}

\subsubsection*{Phase 5: Finalization and Submission (Late March -- Early April)}
\textbf{Planned Activities:}
\begin{itemize}
  \item Stabilize the system and resolve remaining issues
  \item Prepare final documentation and presentation artifacts
  \item Complete final submission deliverables
\end{itemize}

\noindent\textbf{Note on Schedule Completion:}
The submission of the PDF of the poster represents the final
substantive development and delivery milestone for the Reading4All
project. Any tasks scheduled after this point are included solely as
buffer time, administrative placeholders, or contingency periods and
do not correspond to additional system functionality or implementation
phases.
...
\begin{enumerate}[label=MNP-RMNP  \arabic*., wide=0pt, leftmargin=*]
  ...
  \item \emph{The organisation shall operate
...
Table~\ref{tab:evaluation-metrics-summary} summarizes the evaluation metrics that will be used to assess the quality and effectiveness of the alternative text generated by the Reading4All system. Each metric includes its scale type, acceptable range, and a brief description of its purpose.
\begin{table}[H]
    \centering
