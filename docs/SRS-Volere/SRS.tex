...
\begin{table}[H]
    ...
    \begin{tabular}{ |p{3.6cm}|p{7.4cm}|p{3.0cm}| }
      ...
      Uploads images, shows generated alt text, and captures tester evaluations. Provides keyboard/ARIA navigation and WCAG~2.1 Level~A compliant views. &
      Module \\
      \hline
      Backend Controller &
      Orchestrates requests: validates inputs (images/type/size), routes to the Model Service, manages temporary/session storage, records events, and returns results to the UI. &
      Module \\
      \hline
      Model Service &
      Machine-learning inference service that generates alt text from uploaded images and returns text plus timing/quality metadata. Exposed via an internal API. &
      ...
      Deployed application entry point that exposes the Reading4All system to end users over web protocols such as Hyperlink Text Transfer Protocol; integrates Single Sign On (SS0) and serves the User/Application Interfaces. &
...
\item \emph{The tool shall generate alt-text for uploaded images within a reasonable time frame.}\\[2mm] 
    ...
    {\bf Fit Criterion:} The system shall return generated alt-text within \textbf{3 seconds} for images $\leq$ 2 MB and within \textbf{8 seconds} for images $\leq$ 10 MB under normal load conditions.\\
    {\bf Priority:} High
...
\item \emph{The web interface shall load and render accessibility components efficiently.}\\[2mm] 
    ...
    {\bf Fit Criterion:} All interactive elements shall respond within \textbf{300 ms} of user input under typical conditions.\\
    {\bf Priority:} Medium
\end{enumerate}
...
\item \emph{The tool shall not produce alt-text containing offensive, biased, or harmful language.}\\[2mm] 
...
Please refer to Appendix - Evaluation Metrics Summary to understand the evaluation scales and metrics.
...
\item \emph{The generated alt-text shall adequately describe the image content with minimal omissions or irrelevant details.}\\[2mm] 
    ...
    {\bf Fit Criterion:} At least \textbf{85\%} of outputs rated “Sufficient” or better on the sufficiency scale by testers.\\
    {\bf Priority:} High
...
\item \emph{The alt-text shall maintain appropriate length and readability.}\\[2mm] 
    ...
    {\bf Fit Criterion:} $\geq$ 90\% of outputs rated “Proper Length” on the user-testing scale.\\
    {\bf Priority:} Medium
...
\item \emph{The overall accessibility and usability of the alt-text shall be acceptable to testers.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Median user rating $\geq$ 3 (“Mostly Accessible/Usable”) on the 0–3 or 0–4 scales; no outputs below 2.\\
    {\bf Priority:} Medium
\end{enumerate}
...
\item \emph{The system shall gracefully handle unsupported or corrupted image inputs.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Invalid files trigger a clear error message within 2 seconds without interrupting service.\\
    {\bf Priority:} High
...
\item \emph{The backend shall recover automatically from isolated process failures.}\\[2mm] 
    ...
    {\bf Fit Criterion:} System recovers within 5 seconds after fault detection.\\
    {\bf Priority:} High
\end{enumerate}
...
\item \emph{The system shall support limited concurrent usage suitable for a proof-of-concept deployment.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Supports at least \textbf{2 simultaneous requests} with response times $\leq$ 10 seconds.\\
    {\bf Priority:} Medium
...
\item \emph{Storage shall accommodate pilot testing datasets.}\\[2mm] 
    ...
    {\bf Fit Criterion:} The system can temporarily store metadata for up to \textbf{500 images per day} without data loss.\\
    {\bf Priority:} Low
\end{enumerate}
...
\item \emph{The codebase shall be maintainable and adaptable to updates in WCAG guidelines, software libraries, and ML frameworks.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Minor updates or migrations require $\leq$ 2 person-days per quarter.\\
    {\bf Priority:} Medium
...
\item \emph{All communication between the frontend, backend, and machine learning services shall use encrypted HTTPS (TLS 1.2 or higher).}\\[2mm] 
...
\item \emph{Uploaded images shall be deleted immediately after processing is complete.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Uploaded files are stored temporarily in memory or on a secure local directory and deleted within 60 seconds after alt-text generation.\\
    {\bf Priority:} High
...
\item \emph{The system shall validate and sanitize all uploaded files to prevent malicious or unsupported file types.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Only files with valid image types (.png, .jpg, .jpeg, .svg, .webp) are accepted; unsupported or script files are automatically rejected.\\
    {\bf Priority:} High
...
\item \emph{The system shall block access from networks or domains outside McMaster University’s infrastructure.}\\[2mm] 
    ...
    {\bf Fit Criterion:} Requests must originate from verified McMaster SSO tokens or IP ranges associated with university networks.\\
    {\bf Priority:} High
\end{enumerate}
...
\item \emph{Development Effort}\\[2mm] 
    ...
    {\bf Fit Criterion:} Based on a team of four student developers and one faculty supervisor:
    \begin{itemize}
    ...
    \end{itemize}
    {\bf Total Estimated Effort:} 330 hours per team member.\\
    {\bf Priority:} High
...
\item \emph{Testing Environment}\\[2mm] 
    ...
    {\bf Fit Criterion:} Testing is conducted on McMaster-hosted or open-source platforms. User testing sessions may include gift card incentives (\$100–\$150).\\
    {\bf Priority:} Medium
...
\item \emph{Total Development Effort}\\[2mm] 
    ...
    {\bf Fit Criterion:} Approximately \textbf{1,320 person-hours} across four team members (4 × 330 hours).\\
    {\bf Priority:} High

\item \emph{Notional Monetary Cost}\\[2mm] 
    {\bf Rationale:} Provides an academic-equivalent cost estimate for effort valuation.\\
    {\bf Fit Criterion:} Assuming an average rate of \textbf{\$20/hour}, total notional cost is approximately \textbf{\$26,400 CAD}.\\
    {\bf Priority:} Medium
...
\item \emph{Actual Financial Cost}\\[2mm] 
    ...
    {\bf Fit Criterion:} The actual monetary cost is \textbf{\$0 CAD}; all expenditure is in research and development effort.\\
    {\bf Priority:} High
\end{enumerate}
